...
/* filter seleced project in category page*/
$(document).ready(function () {
    ...
    filter_Item.click(function () {
        ...
        var Idttribute = $(this).attr('data-Idttribute');
        var filer_selected = $('.filer_selected');
        var filer_selected_div = filer_selected.find('div[data-id=' + id + ']');
        //var filer_selected_div = filer_selected.find('div[Idttribute=' + Idttribute + ']');
        ...
        if (len > 0) {
            ...
            var filter_content = '<div data-id="' + id + '" class="filer_selected_items border bg-light mx-1"><span>' + title + ':' + value + '</span><i class= "fal fa-times float-left" onclick="removeSelected(this)"></i><input type="hidden" name="attr-' + Idttribute + '[]" value="' + id + '"></div>';
...
$(document).ready(function () {
    ...
    Filter.hover(function () {
        $('.filter_list', this).slideDown(0);

    }, function () {
        $('.filter_list', this).slideUp(0);
    })
...
/* step order */
$(document).ready(function () {
    var step_Item = $('.step_order2 .row .col-1');
    step_Item.click(function () {
        $('.circle', this).toggleClass('active_check');
    })
})
...
function show_description(input) {
    var itag = $(input)
    itag.toggleClass('active');
    var paretnt_tr = itag.parents('tr');
    paretnt_tr.next().fadeToggle(120);
}


/* Update_QuantityBasket*/
